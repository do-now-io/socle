...
locals {
  ...
  # CoreDNS and the Flux operator are Deployments: with no node, Helm would
  # wait helm_timeout_seconds for pods that never schedule, then fail
  # without saying why.
  no_node_message = "The cluster has no schedulable node (schedulable_nodes = 0), so CoreDNS and the Flux operator cannot start. On aws the foundations' bootstrap_node_count sets it; it must be at least 1."
