...
| <a name="input_kubernetes_version"></a> [kubernetes\_version](#input\_kubernetes\_version) | AKS control plane version. The default tracks the same n-1 policy ceiling as the other clouds — not the newest AKS offers, and not one close to the end of its standard support. | `string` | `"1.34"` | no |
